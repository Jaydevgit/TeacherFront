    ...
    data() {
      return {
        ...
        options: {
          ...
          autoCropWidth: 428,  //截图框宽度
          autoCropHeight: 270, //截图框高度
    ...
    methods: {
      transportMessage(msg,name){
        ...
        switch (msg) {
          case 'front':
            ...
            break;
          case 'back':
            this.titleName = '上传身份证反面'
            this.options.autoCropWidth = 428
            this.options.autoCropHeight = 270
            break;
          case 'working':
            this.titleName = '上传在职证明'
            this.options.autoCropWidth = 428
            this.options.autoCropHeight = 270
            break;
          case 'logo': {
            this.titleName = '学校Logo'
            this.options.autoCropWidth = 120
            this.options.autoCropHeight = 120
            break;
          }
          case 'unit':{
            this.titleName = '更换学校Logo'
            this.options.autoCropWidth = 120
            this.options.autoCropHeight = 120
            break;
          }
          case 'background':{
            this.titleName = '更换背景图片'
            this.options.autoCropWidth = 420
      ...
      cut(type) {
        ...
        this.$refs.cropper.getCropBlob(res => {
          //res是裁剪后图片的bolb对象
          var uploadName = this.msg+'.png';
          formData.append("file", res, uploadName);
          var src = window.URL.createObjectURL(res);


          console.log("图片路径为: " + src);
          this.attach.laterUrl = src;
          this.axios.put('/api/attach/upload/certificate', formData,
...
<style>
  ...
  .el-dialog{
    border-radius: 15px;
    box-shadow: 0 0 14px 1px antiquewhite;
  }
</style>
