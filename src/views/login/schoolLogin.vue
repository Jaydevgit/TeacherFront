...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleLogin() {
        if(this.picLyanzhengma.toUpperCase() !==this.identifyCode.toUpperCase() ){
          ...
          this.$refs.loginForm.validate(valid => {
            if (valid ) {
              this.loading = true
              this.loginForm.host = 'school'
              this.$store.dispatch('schoolLogin', this.loginForm).then(data => {
                this.loading = false
                console.log(JSON.stringify(data));
                if ("success" === data.result) {
                  this.$router.push({path: '/school/manager'})
                } else {
      ...
      toUnitApply() {
        this.$router.push({name: 'unitApplySchool'});
      },
