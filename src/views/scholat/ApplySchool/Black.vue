<template>
  <div class="app-container">
    ...
    <el-table :data="list"
      ...
      <el-table-column align="left" label="申请单位" width="200">
        <template slot-scope="scope">
          <div>
            <span class="label-describe">申请学校</span><span>{{scope.row.school_name}}</span>
          </div>
          <div>
            <span class="label-describe">申请单位</span><span>{{scope.row.unit_name}}</span>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" align="center" label="管理" v-if="hasScholatPerm('unit:update')">
        <template slot-scope="scope">
          <el-button type="success" size="small" @click="cancelBlackApply(scope.row.id)">从黑名单中移除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    methods: {
      cancelBlackApply(id){
        ...
        this.$confirm('确定从黑名单中移除?', '提示', {
          ...
          _vue.api({
            url: "/scholat/apply/cancelBlackApplySchool",
            method: "post",
            data: {
              "apply_id":id
            }
          }).then((data) => {
            this.$message.success("从黑名单中移除成功")
            this.getList();
          }).catch(e => {
            this.$message.error("从黑名单中移除失败")
          })
        })
      },
      ...
      * */
      getList() {
        ...
        this.api({
          url: "/scholat/apply/getInfoSchool",
          method: "get",
